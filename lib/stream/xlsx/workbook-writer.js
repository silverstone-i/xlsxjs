const fs = require("fs");
const { ZipWriter } = require("../../utils/zip-stream");
...
class WorkbookWriter {
  constructor(options) {
    ...
    // pending streams from _openStream() to be flushed before finalize
  ...
  _openStream(path) {
    ...
    // Track stream for later append to zip during commit
    this._pendingStreams.push({ stream, path });
    stream.on("finish", () => {
      stream.emit("zipped");
    });
    return stream;
  ...
  _flushPendingStreams() {
    for (const { stream, path } of this._pendingStreams) {
      const data = stream.toBuffer();
      if (data && data.length) {
