...
class WorkbookReader extends EventEmitter {
  ...
  async *parse(input, options) {
    ...
    const inputSource = input || this.input;
    const inputPath =
      typeof inputSource === 'string'
        ? inputSource
        : inputSource && typeof inputSource.path === 'string'
          ? inputSource.path
          : null;

    if (inputPath) {
      yield* this._parseFromFile(inputPath);
      return;
    }

    const stream = (this.stream = this._getStream(inputSource));
    ...
    for await (const entry of iterateStream(zip)) {
      ...
      switch (entryPath) {
        ...
        case 'xl/styles.xml':
          ...
          break;
        default:
          if (entryPath.toLowerCase().endsWith('sharedstrings.xml')) {
            yield* this._parseSharedStrings(entry);
            break;
          }
          if (entryPath.match(/xl\/worksheets\/sheet\d+[.]xml/)) {
            ...
            entryPath.match(/xl\/worksheets\/_rels\/sheet\d+[.]xml.rels/)
          ) {
            match = entryPath.match(
  ...
  async *_parseFromFile(filename) {
    const directory = await unzip.Open.file(filename);
    ...
    for (const file of directory.files) {
      ...
      switch (entryPath) {
        case '_rels/.rels':
          await drainStream(entryStream);
          break;
        case 'xl/_rels/workbook.xml.rels':
          await this._parseRels(entryStream);
          break;
        case 'xl/workbook.xml':
          await this._parseWorkbook(entryStream);
          break;
        case 'xl/sharedStrings.xml':
          yield* this._parseSharedStrings(entryStream);
          break;
        case 'xl/styles.xml':
          await this._parseStyles(entryStream);
          break;
        default:
          if (entryPath.toLowerCase().endsWith('sharedstrings.xml')) {
            yield* this._parseSharedStrings(entryStream);
          ...
          if (entryPath.match(/xl\/worksheets\/sheet\d+[.]xml/)) {
            ...
            sheetNo = match[1];
            if (this.sharedStrings && this.workbookRels) {
              yield* this._parseWorksheet(iterateStream(entryStream), sheetNo);
            } else {
              await new Promise((resolve, reject) => {
                tmp.file((err, path, fd, tempFileCleanupCallback) => {
                  if (err) {
            ...
            sheetNo = match[1];
            yield* this._parseHyperlinks(iterateStream(entryStream), sheetNo);
          } else {
            await drainStream(entryStream);
          }
          break;
