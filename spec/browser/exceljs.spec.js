...
describe('ExcelJS', () => {
  ...
  it('should hash worksheet protection using browser crypto', done => {
    const wb = new ExcelJS.Workbook();
    const ws = wb.addWorksheet('secure');

    ws.protect('secret', {spinCount: 1000})
      .then(() => {
        expect(ws.sheetProtection).toBeTruthy();
        expect(ws.sheetProtection.algorithmName).toBeTruthy();
